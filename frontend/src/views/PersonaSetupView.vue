    ...
    <div class="max-w-4xl mx-auto">
      ...
      <n-card v-if="personaStore.hasPersona" class="mb-4">
        <div class="flex items-center justify-between">
          ...
          <n-progress
            type="circle"
            :percentage="personaStore.completeness"
        ...
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <n-form-item label="称呼/昵称" path="preferred_name">
              <n-input
                v-model:value="formData.preferred_name"
                placeholder="小张、张老师等"
                clearable
              />
            </n-form-item>
            ...
            <n-form-item label="年龄段" path="age_range">
              <n-select
                v-model:value="formData.age_range"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <n-form-item label="正式程度" path="formality">
              <n-select
                v-model:value="formData.formality"
            ...
            <n-form-item label="回答详细度" path="detail_level">
              <n-select
                v-model:value="formData.detail_level"
            ...
            <n-form-item label="性格类型" path="personality_type">
              <n-input
                v-model:value="formData.personality_type"
                placeholder="如：INTJ、外向型等"
                clearable
              />
            </n-form-item>
          </div>
          ...
          <n-form-item label="幽默程度" path="humor_level">
            <div class="w-full">
              <div class="flex items-center gap-4 mb-2">
                <div class="flex-1">
                  <n-slider
                    v-model:value="formData.humor_level"
                    :min="1"
                    :max="10"
                    :step="1"
                ...
                <div class="flex items-center justify-center w-12 h-10 bg-primary-50 dark:bg-primary-900/20 rounded-lg">
                  <span class="text-lg font-bold text-primary-600 dark:text-primary-400">{{ formData.humor_level }}</span>
                </div>
              </div>
            </div>
          </n-form-item>
          ...
          <n-form-item label="沟通风格描述" path="communication_style">
            <n-input
              v-model:value="formData.communication_style"
              type="textarea"
              :rows="3"
              placeholder="例如：我喜欢用比喻来解释复杂概念，说话时会经常反问，喜欢用'其实'开头..."
              show-count
            />
          </n-form-item>
          ...
          <n-form-item label="工作经验" path="work_experience">
            <n-input
              v-model:value="formData.work_experience"
              type="textarea"
              :rows="3"
              placeholder="简要描述你的工作经历和主要项目..."
              show-count
            />
          </n-form-item>
          ...
          <n-form-item label="个人观点" path="personal_views">
            <div class="space-y-2 w-full">
              <div
                v-for="(view, topic) in formData.personal_views"
                :key="topic"
                class="flex gap-2"
              >
                <n-input
                ...
                />
                <n-button
                  @click="removePersonalView(topic)"
                  type="error"
                  secondary
                >
                  删除
              ...
              <div class="grid grid-cols-[auto_1fr_auto] gap-2 items-start">
                <n-input
                  v-model:value="newViewTopic"
                  placeholder="话题"
                  class="w-32"
                />
                <n-input
                  v-model:value="newViewContent"
                  type="textarea"
                  :rows="3"
                  placeholder="你的观点"
                />
                <n-button
                  @click="addPersonalView"
                  type="primary"
                >
                  添加
                </n-button>
          ...
          <n-form-item label="问答示例" path="faq_examples">
            <div class="space-y-2 w-full">
              <div
                v-for="(example, index) in formData.faq_examples"
                :key="index"
              ...
              <div class="border rounded p-3 space-y-2">
                ...
                <n-input
                  v-model:value="newFAQQuestion"
                  placeholder="问题"
                />
                <n-input
                  v-model:value="newFAQAnswer"
                  type="textarea"
                  :rows="2"
                  placeholder="你的回答"
                />
          ...
          <n-form-item label="补充信息" path="background_info">
            <n-input
              v-model:value="formData.background_info"
              type="textarea"
              :rows="4"
              placeholder="任何你希望AI了解的额外信息..."
              show-count
            />
          </n-form-item>
          ...
          <n-form-item label="自定义指令" path="custom_instructions">
            <n-input
              v-model:value="formData.custom_instructions"
              type="textarea"
              :rows="3"
              placeholder="给AI的额外指令，会附加到系统提示词中..."
              show-count
            />
          </n-form-item>
