<template>
  <header class="sticky top-0 z-50 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 safe-top">
    <div class="flex items-center justify-between h-14 sm:h-16 px-4">
      <!-- Left Section -->
      <div class="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
        <!-- Menu Button (mobile) -->
        <button v-if="showMenuButton" @click="$emit('menu-click')"
        ...
        <!-- Back Button -->
        <button v-if="showBackButton" @click="$emit('back-click')"
      ...
      <div class="flex items-center gap-1 sm:gap-2 flex-shrink-0">
        ...
        <!-- Settings Button -->
        <button v-if="showSettings" @click="$emit('settings-click')"
          class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors btn-touch" aria-label="设置">
          <svg class="w-5 h-5 sm:w-6 sm:h-6 text-gray-700 dark:text-gray-300" fill="none" stroke="currentColor"
            viewBox="0 0 24 24">
        ...
        <div v-if="showUserMenu" class="relative">
          <button @click.stop="toggleUserMenu"
            class="p-1 rounded-full hover:ring-2 hover:ring-primary-500 transition-all btn-touch" aria-label="用户菜单">
            <img v-if="userAvatar" :src="userAvatar" :alt="username"
              class="w-8 h-8 sm:w-9 sm:h-9 rounded-full object-cover" />
            <div v-else class="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-primary-500 flex items-center justify-center">
