<template>
  <div class="border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 safe-bottom">
    <div class="max-w-4xl mx-auto px-4 py-3 sm:py-4">
      ...
      <div v-if="showConfig || !isMobile" class="mb-3 transition-all duration-300"
        ...
        <div class="flex items-center gap-2 sm:gap-3 flex-wrap">
          ...
          <button type="button" @click="toggleThinkMode" :disabled="disabled || !isClaudeModel"
            :title="!isClaudeModel ? t('chat.input.thinkModeHint') : ''"
            class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all duration-200" :class="[
            ...
            <span v-if="localConfig.thinkMode"
              class="ml-0.5 w-1.5 h-1.5 bg-amber-600 dark:bg-amber-400 rounded-full"></span>
          </button>

          <!-- Web Search Toggle Button -->
          <button type="button" @click="toggleWebSearch" :disabled="disabled"
            class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all duration-200" :class="[
            ...
            <span v-if="localConfig.webSearch"
              class="ml-0.5 w-1.5 h-1.5 bg-blue-600 dark:bg-blue-400 rounded-full"></span>
          </button>
          ...
          <button @click="$refs.fileInputRef.click()" :disabled="disabled"
            ...
            <svg class="w-4 h-4 text-gray-500 dark:text-gray-400 group-hover:text-primary-500" fill="none"
              stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
            </svg>
            <span class="text-xs sm:text-sm text-gray-700 dark:text-gray-300 select-none">{{ t('chat.input.attachFile')
              }}</span>
            <!-- File count badge -->
            <span v-if="attachedFiles.length > 0"
          ...
          <button v-if="isMobile" @click="toggleConfig"
      ...
      <div v-if="isMobile && !showConfig" class="mb-2 flex justify-center">
      ...
      <div class="flex items-end gap-2 sm:gap-3">
        ...
        <div class="flex-1 relative">
          ...
          <div v-if="showCharCount && maxLength"
            class="absolute bottom-3 right-3 text-xs select-none pointer-events-none" :class="{
        ...
        <button v-if="!canStop" @click="handleSend" :disabled="!canSend"
          ...
          <!-- Send Icon -->
          <svg v-if="!sending" class="w-5 h-5 sm:w-6 sm:h-6 transform transition-transform group-hover:scale-110"
            fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="flex items-center justify-between mt-2 px-1">
        <p class="text-xs text-gray-400 dark:text-gray-500">
          <kbd
            class="px-1.5 py-0.5 text-xs bg-gray-100 dark:bg-gray-700 rounded border border-gray-300 dark:border-gray-600">Enter</kbd>
          {{ t('chat.input.sendHint') }}
          <span class="mx-1">•</span>
          <kbd
            class="px-1.5 py-0.5 text-xs bg-gray-100 dark:bg-gray-700 rounded border border-gray-300 dark:border-gray-600">Shift+Enter</kbd>
