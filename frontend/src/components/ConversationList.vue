    ...
    <div class="flex-shrink-0 px-4 py-4 border-b border-gray-200 dark:border-gray-700">
      <div class="flex items-center justify-between mb-3">
        <h2 class="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">
          {{ t('chat.conversationList.title') }}
        </h2>
        <button @click="createNewConversation" :disabled="loading"
          ...
          :title="t('chat.conversationList.newChat')">
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar min-h-0">
      ...
      <div v-else-if="filteredConversations.length === 0" class="p-8 text-center">
        ...
        <p class="mt-4 text-gray-500 dark:text-gray-400">
          {{ t('chat.conversationList.noConversations') }}
        </p>
      </div>
        ...
        <div v-for="conversation in filteredConversations" :key="conversation.id"
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
              <button @click.stop="confirmDelete(conversation.id)"
      ...
      <div v-if="showDeleteModal"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all"
          ...
          <h3 class="text-lg font-semibold text-center text-gray-900 dark:text-white mb-2">
            {{ t('chat.conversationList.deleteTitle') }}
          </h3>

          <!-- Message -->
          <p class="text-sm text-center text-gray-600 dark:text-gray-400 mb-6">
            {{ t('chat.conversationList.deleteConfirm') }}
          </p>
...
const formatTime = (dateString) => {
  ...
  if (diffMins < 1) return isEnglish ? 'Just now' : '刚刚'
  if (diffMins < 60) return isEnglish ? `${diffMins} min ago` : `${diffMins} 分钟前`

  const diffHours = Math.floor(diffMins / 60)
  if (diffHours < 24) return isEnglish ? `${diffHours} hr ago` : `${diffHours} 小时前`

  const diffDays = Math.floor(diffHours / 24)
  if (diffDays < 7) return isEnglish ? `${diffDays} day${diffDays > 1 ? 's' : ''} ago` : `${diffDays} 天前`

  const isSameYear = date.getFullYear() === now.getFullYear()
  const localeStr = isEnglish ? 'en-US' : 'zh-CN'
