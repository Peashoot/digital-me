/**
 * Doubao Model Adapter (ByteDance Volcengine)
 * 豆包使用火山引擎的 OpenAI 兼容 API
 */
...
export class DoubaoAdapter extends BaseModelAdapter {
  ...
  async streamGenerate(
    ...
    const requestBody = {
      model: this.config.model, // 这里使用 endpoint_id
    ...
    if (!response.ok) {
      ...
      throw new Error(`Doubao API error: ${JSON.stringify(error)}`)
    }
        ...
        for (const line of lines) {
          if (line.startsWith('data:')) {
            ...
            try {
              ...
              console.error('Error parsing Doubao SSE:', e)
            }
          }
        }
      }
