/**
 * Hunyuan Model Adapter (Tencent Cloud)
 * 腾讯混元使用 OpenAI 兼容的 API
 */

import { BaseModelAdapter, Message, ModelConfig, ModelResponse, StreamChunk } from './base.ts'

export class HunyuanAdapter extends BaseModelAdapter {
  private endpoint = 'https://api.hunyuan.cloud.tencent.com/v1/chat/completions'
  ...
  async streamGenerate(
    ...
    if (!response.ok) {
      ...
      throw new Error(`Hunyuan API error: ${JSON.stringify(error)}`)
    }
        ...
        for (const line of lines) {
          if (line.startsWith('data:')) {
            ...
            try {
              ...
              console.error('Error parsing Hunyuan SSE:', e)
            }
          }
        }
      }
